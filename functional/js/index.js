...
// console.log(BABYNAMES.filter(isMale));
...
function byName(record1, record2) {
    // compare record1 with record2 and adjust locations based on value given
...
function descending(comparator) {
    return function(record1, record2) {
        // ... negate the resule of the comparator()
...
let top10FemalesNames = BABYNAMES.filter(isFemale)
.sort(byCountDescending)
.slice(0, 10)
.map(pluckName);

console.log(top10FemalesNames);

function toLower(string) {
    return string.toLowerCase();
}
let top10FemalesNamesLower = top10FemalesNames.map(toLower);

console.log(top10FemalesNamesLower)
...
function max (n1, n2) {
    // ternary conditional -- look at Intro to JS tutorial for more
    return n2 > n1 ? n2 : 1; // If n2 > n1 then return n2, if not return n1
}
...
let maxNum = someNum.reduce(min, 72);
console.log(maxNum);
